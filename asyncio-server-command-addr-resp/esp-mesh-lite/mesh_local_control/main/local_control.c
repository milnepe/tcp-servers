...
void tcp_client_read_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "TCP client read task is running");
    ...
    while (1) {
        ...
        len = recv(g_sockfd, rx_buffer, sizeof(rx_buffer) - 1, 0);
        if (len < 0) {
            ...
            // Set LED according to received command
            if (strcmp(rx_buffer, "ON\r\n") == 0) {
                s_led_state = 1;
                // gpio_set_level(BLINK_GPIO, s_led_state);
                ESP_LOGI(TAG, "GPIO %d turned ON", BLINK_GPIO);                
                // if (s_led_state) {
                //     ESP_LOGI(TAG, "GPIO %d turned ON", BLINK_GPIO);
                // } else {
                //     ESP_LOGI(TAG, "GPIO %d turned OFF", BLINK_GPIO);
                // }
    ...
    ESP_LOGI(TAG, "TCP client read task is exit");
    close(g_sockfd);
    g_sockfd = -1;
    vTaskDelete(NULL);
}
...
void app_main()
{
    ...
    xTaskCreate(tcp_client_read_task, "tcp_client_read_task", 4096, NULL, 5, NULL);
